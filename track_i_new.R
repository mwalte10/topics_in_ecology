...
susceptible_init_h[,3] <- c(initial_conditions[1:28,3] * (0.1 * 12 * 10^6), initial_conditions[29:56,3] * 0, 
...
susceptible_init_l[,3] <- c(initial_conditions[1:28,3] * (0.9 * 12 * 10^6), initial_conditions[29:56,3] * 0, 
                            initial_conditions[57:84,3] * 0, initial_conditions[85:112,3] * 0)
...
{track_infected.null <- out_null[,(ncol(out_null) - 3)]
  ...
  cases.null <- cases[length(cases.null)]
...
save(sp9, file = paste('sp9.low_', input, '.RData', sep = ''))
