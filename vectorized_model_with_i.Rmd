...
population <- sum(y)
    ...
    dS1 <-  
      ...
      + c(0, (1/365) * (1/(head(age_window, -1)))) * c(head(S1, -1), 0)
    
    dI1 <- 
      S1 * (beta * infected_total)
      + c(0, (1/365) * (1/(head(age_window, -1)))) * c(head(I1, -1), 0)
      - (1/365) * (1/age_window) * c(head(I1, - 1), 0)
      - I1 * (gamma + delta)
    
    dR1 <-
      I1 * gamma 
      + c(0, (1/365) * (1/(head(age_window, -1)))) * c(head(R1, -1), 0)
    ...
    dI2 <- 
      ...
      + c(0, (1/365) * (1/(head(age_window, -1)))) * c(head(I2, -1), 0)
      - (1/365) * (1/age_window) * c(head(I2, - 1), 0)
      - I2 * (gamma + delta)
    
    dR2 <-  
      I2 * gamma 
      + c(0, (1/365) * (1/(head(age_window, -1)))) * c(head(R2, -1), 0)
    ...
    dI3 <- 
      ...
      + c(0, (1/365) * (1/(head(age_window, -1)))) * c(head(I3, -1), 0)
      - (1/365) * (1/age_window) * c(head(I3, - 1), 0)
      - I3 * (gamma + delta)
    
    dR3 <- 
      I3 * gamma 
      + c(0, (1/365) * (1/(head(age_window, -1)))) * c(head(R3, -1), 0)
    ...
    dI4 <- 
      ...
      + c(0, (1/365) * (1/(head(age_window, -1)))) * c(head(I4, -1), 0)
      - (1/365) * (1/age_window) * c(head(I4, - 1), 0)
      - I4 * (gamma + delta)
    
    dR4 <- 
      I4 * gamma 
      + c(0, (1/365) * (1/(head(age_window, -1)))) * c(head(R4, -1), 0)
...
titles <- c(rep("SUSCEPTIBLE(1)", 28), rep("INFECTED(1)", 28), rep("RECOVERED(1)", 28),
            rep("SUSCEPTIBLE(2)", 28), rep("INFECTED(2)", 28), rep("RECOVERED(2)", 28),
          rep("SUSCEPTIBLE(3)", 28), rep("INFECTED(3)", 28), rep("RECOVERED(3)", 28),
            rep("SUSCEPTIBLE(4)", 28), rep("INFECTED(4)", 28), rep("RECOVERED(4)", 28))
plot(out, main = titles)
View(out)
```


```{r}
sum_mat <- matrix(NA, nrow = 36501, ncol = 12)

for(i in 1:12){
  for(j in 1:36501){
    if(i != 1){
      x <- rep(i * 28, 28)
      sum_mat[j,i+1] <- sum(out[j,x + (2:29)])
    }else{
       sum_mat[j,i+1] <- sum(out[j,2:29])
    }
  }
}

titles <- c("susceptible_1", "infected_1", "recovered_1",
            "susceptible_2", "infected_2", "recovered_2",
            "susceptible_3", "infected_3", "recovered_3",
            "susceptible_4", "infected_4", "recovered_4")

##plots of sums
for(i in 1:12){
  plot(sum_mat[,i], main = titles[i], type = "l")
}
