...
parms_vac <- c(beta = 1,
          ...
          delta = c((0.013 / 365), rep(0.001 / 365, 4), rep(0, 10), rep(0.001, 5), 
                    ...
                    0.114 / 365, 0.151 / 365),
          age_window = c(rep(1, 21), rep(10, 7)),
          vac = c(rep(0,8), 0.5, rep(0.2, 8), rep(0, 11)))
...
for(i in 0:3){
  x <- 2 + 3 * i
  {plot(out_vac[,1],rowSums(out_vac[,(1+(x-1)*28+1):(1+(x)*28)]),type='l', 
      lty= 2, col = "red", xlim = range(0:1300),
      ylim= range(0:3.5e5), main = infected_names[i + 1])
  lines(out_mod[,1],rowSums(out_mod[,(1+(x-1)*28+1):(1+(x)*28)]),type='l')
  legend(600, 2e5, legend=c("Vaccination", "No Vaccination"),
...
for(i in 0:3){
  ...
  {plot(out_vac[,1],rowSums(out_vac[,(1+(x-1)*28+1):(1+(x)*28)]),type='l', 
      lty= 2, col = "red", xlim = range(0:1300),
      ylim= range(0:5.5e5), main = susceptible_names[i + 1])
  lines(out_mod[,1],rowSums(out_mod[,(1+(x-1)*28+1):(1+(x)*28)]),type='l')
  legend(1000, 4e5, legend=c("Vaccination", "No Vaccination"),
       col=c("red", "black"), lty = c(2,1), cex=0.8)}}
...
for(i in 0:3){
  ...
  {plot(out_vac[,1],rowSums(out_vac[,(1+(x-1)*28+1):(1+(x)*28)]),type='l', 
      lty= 2, col = "red", xlim = range(0:1300),
      ylim= range(0:9e5), main = recovered_names [i + 1])
  lines(out_mod[,1],rowSums(out_mod[,(1+(x-1)*28+1):(1+(x)*28)]),type='l')
  legend(1000, 4e5, legend=c("Vaccination", "No Vaccination"),
       col=c("red", "black"), lty = c(2,1), cex=0.8)}}
