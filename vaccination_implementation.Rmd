...
parms_vac <- c(beta = 3e-1,
          gamma = 2.5e-1,
          sigma = 1/(365 * 1.2),
          mu = 19 / (1000 * 365),
          delta = 8 / (1000 * 365),
          age_window = c(rep(1, 21), rep(10, 7)),
          vac = c(rep(0,8), 1, rep(0.2, 8), rep(0, 11)))
```

```{r Model, no vaccination}
...
out_mod <- ode(times = times, y = y_init, func = model, parms = parms)
out_vac <- ode(times = times, y = y_init, func = model_vac, parms = parms_vac)
```

```{r graphs}
#plot infected, from vaccination model 
{plot(out_vac[,1],rowSums(out_vac[,(1+(2-1)*28+1):(1+(2)*28)]),type='l', 
      lty= 2,
      ylim= range(0:1e6), main = "Infected, Vaccination Model")
lines(out_vac[,1],rowSums(out_vac[,(1+(5-1)*28+1):(1+(5)*28)]),type='l', lty= 2, col ="red")
lines(out_vac[,1],rowSums(out_vac[,(1+(8-1)*28+1):(1+(8)*28)]),type='l', lty= 2, col ="blue")
lines(out_vac[,1],rowSums(out_vac[,(1+(11-1)*28+1):(1+(11)*28)]),type='l', lty= 2, col ="green")
lines(out_mod[,1],rowSums(out_mod[,(1+(2-1)*28+1):(1+(2)*28)]),type='l')
lines(out_mod[,1],rowSums(out_mod[,(1+(5-1)*28+1):(1+(5)*28)]),type='l', col ="red")
lines(out_mod[,1],rowSums(out_mod[,(1+(8-1)*28+1):(1+(8)*28)]),type='l', col ="blue")
lines(out_mod[,1],rowSums(out_mod[,(1+(11-1)*28+1):(1+(11)*28)]),type='l', col ="green")
legend(600, 8.05e5, legend=c("Infected 1", "Infected 2", "Infected 3", "Infected 4"),
       col=c("black","red", "blue","green"), lty = 2, cex=0.8)
