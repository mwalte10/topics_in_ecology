...
# betas <- seq(0.15, 0.25, length.out = 30)
# beta_l <- rep(betas, 30)
# beta_h <- list()
# for(i in 1:30){
#   beta_h[[i]] <- rep(betas[i], 30)
# }
# beta_h <- unlist(beta_h)
# travel_h <- rep(0.0025, 900)
# travel_l <- rep(0.025, 900)
# native_h <- 1 - travel_h
# native_l <- 1 - travel_l
# parms.mat <- cbind(beta_l, beta_h, travel_l, travel_h, native_l, native_h)
# remove <- c()
# for(i in 1:900){
...
# native_l <- parms.mat[input, 5]
# native_h <- parms.mat[input, 6]
...
model <- function(t, y, parms){
  ...
  FOI_l.travel <-
    sum(native_l *  beta_l * 2 * 0.75 * (native_l * (sym_inf_l) / pop_l + travel_h * (sym_inf_h)  / pop_h) +
          travel_l * beta_h * 2 *  0.75 * (native_h * (sym_inf_h)  / pop_h + travel_l * (sym_inf_l) / pop_l) +
          native_l * beta_l * 0.75 * (native_l * (inf_l) / pop_l + travel_h * (inf_h)  / pop_h) +
          travel_l * beta_h * 0.75 * (native_h * (inf_h)  / pop_h + travel_l * (inf_l) / pop_l))
  # 
  # FOI_h <-
  #   sum(2 * beta_h * (native * (sym_inf_h) / pop_h + travel * (sym_inf_l) / pop_l) +
  #         2 * beta_l * (native * (sym_inf_l) / pop_l + travel * (sym_inf_h) / pop_h) +
  #         beta_h * (native * (inf_h) / pop_h + travel * (inf_l) / pop_l) +
  #         beta_l * (native * (inf_l) / pop_l + travel * (inf_h) / pop_h))
  # 
  # FOI_l <-
  #   sum(2 * beta_l * (native * (sym_inf_l) / pop_l + travel * (sym_inf_h) / pop_h) +
  #         2 * beta_h * (native * (sym_inf_h)/ pop_h + travel * (sym_inf_l) / pop_l) +
  #         beta_l * (native * (inf_l) / pop_l + travel * (inf_h) / pop_h) +
  #         beta_h * (native * (inf_h)/ pop_h + travel * (inf_l) / pop_l))
  # 
...
for(i in (years_vac * 3650):(years * 3650)){
  prop.h[[i]] <- out.h[i,which(colnames(out.h) == 'prop_h')] / sum(out.h[i,which(colnames(out.h) == 'prop_h')])
  prop.l[[i]] <- out.h[i,which(colnames(out.h) == 'prop_l')] / sum(out.h[i,which(colnames(out.h) == 'prop_l')] )
}
...
# FOI_h.travel <- sum(diff(out.h[indexing,which(colnames(out.h) == 'FOI_h.travel')]) / (length(indexing)))
# FOI_l.travel <- sum(diff(out.h[indexing,which(colnames(out.h) == 'FOI_l.travel')]) / (length(indexing)))
...
cases_averted.func <- function(out_mat, out_mat_null, timepoint_year){
  ...
  pop_h <- sum(out_mat[(timepoint_year * 3650),((which(colnames(out_mat) == 'sh1')[1]):(which(colnames(out_mat) == 'rh4'))[length(which(colnames(out_mat) == 'rh4'))])])
  ...
  pop_l <- sum(out_mat_null[(timepoint_year * 3650),((which(colnames(out_mat) == 'sl1')[1]):(which(colnames(out_mat) == 'rl4'))[length(which(colnames(out_mat) == 'rl4'))])])
  coverage_h <- vac_h / pop_h
  coverage_l <- vac_l / pop_l
  coverage_pop <- (vac_h + vac_l) / (pop_h + pop_l)
