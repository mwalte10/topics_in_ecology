...
model <- function(t, y, parms){
  ...
  for(i in 1:29){
    #first infection
    dS1 <-  susceptible(1)[i] * (-beta * sum(infected_total) - delta) - (1/365) * (1/age_window[i]) * c(head(susceptible(1), - 1), 0) + mu * c(population, rep(0,28)) + c(0, (1/365) * (1/(head(age_window, -1)))) * head(susceptible(1), -1)
      -beta * susceptible(1)[i] * sum(infected_total) + mu * population - delta * susceptible(1)[i]
    dI1 <- susceptible(1)[i] * (beta * sum(infected_total)) - infected(1)[i] * (gamma + delta)
    dR1 <- infected(1)[i] * gamma - recovered(1)[i] * (delta + sigma)
    
    #second infection
    dS2 <- susceptible(2)[i] * ((-3/4) * beta * sum(infected_total) - delta) - (1/365) * (1/age_window[i])*c(head(susceptible(2), - 1), 0) + c(0, (1/365)*(1/(head(age_window, -1)))) * head(susceptible(2), -1) + sigma * recovered(1)[i] 
    dI2 <- susceptible(2)[i] * ((3/4) * beta * sum(infected_total)) - infected(2)[i] * (gamma + delta)
    dR2 <-  infected(2)[i] * gamma - recovered(2)[i] * (delta + sigma)
    
    #third infection
    dS3 <- susceptible(3)[i] * ((-2/4) * beta * sum(infected_total) - delta) - (1/365) * (1/age_window[i])*c(head(susceptible(3), - 1), 0) + c(0, (1/365)*(1/(head(age_window, -1)))) * head(susceptible(3), -1) + sigma * recovered(2)[i] 
    dI3 <- susceptible(3)[i] * ((2/4) * beta * sum(infected_total)) - infected(3)[i] * (gamma + delta)
    dR3 <- infected(3)[i] * gamma - recovered(3)[i] * (delta + sigma) 
    
    #fourth infection
    dS4 <- susceptible(4)[i] * ((-1/4) * beta * sum(infected_total) - delta) - (1/365) * (1/age_window[i])*c(head(susceptible(4), - 1), 0) + c(0, (1/365)*(1/(head(age_window, -1)))) * head(susceptible(4), -1) + sigma * recovered(3)[i] 
    dI4 <- susceptible(4)[i] * ((1/4) * beta * sum(infected_total)) - infected(4)[i] * (gamma + delta)
    dR4 <- infected(4)[i] * gamma - recovered(4)[i] * (delta) 
