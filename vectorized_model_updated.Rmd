...
```{r travel}
travel <- c(rep(1, 7), rep(0.86, 9), rep(0.842, 4), 0.814, 0.7676, 0.7784, rep(0.809, 5))
```
...
```{r rainbow charts susceptible breakdown beta}
total_susceptible <- sum(out[final, 2:29] + out[final, 86:113] + out[final, 170:197] + out[final, 254:281]) 
prop_one <- out[final, 2:29] / total_susceptible
prop_two <- out[final, 86:113] / total_susceptible
prop_three <- out[final, 170:197] / total_susceptible
prop_four <- out[final, 254:281] / total_susceptible

susceptible_table <- matrix(NA, ncol = 4, nrow = 28)
susceptible_table[,1] <- prop_one 
susceptible_table[,2] <- prop_two
susceptible_table[,3] <- prop_three
susceptible_table[,4] <- prop_four

{barplot(susceptible_table, col = rainbow(28), main = "Proportion of Susceptible Groups by Age Group, Beta = 5", ylim = c(0,0.5),
         names.arg = c("Susceptible 1", "Susceptible 2", "Susceptible 3", "Susceptible 4"))
  axis(2, 0:1)
  box()}


```
```{r rainbow charts susceptible breakdown beta_l}
total_susceptible <- sum(out[final, 338:365] + out[final, 422:449] + out[final, 506:533] + out[final, 590:617]) 
prop_one <- out[final, 338:365] / total_susceptible
prop_two <- out[final, 422:449] / total_susceptible
prop_three <- out[final, 506:533] / total_susceptible
prop_four <- out[final, 590:617] / total_susceptible
...
{barplot(susceptible_table, col = rainbow(28), main = "Proportion of Susceptible Groups by Age Group, Beta = 10", ylim = c(0,0.5),
         names.arg = c("Susceptible 1", "Susceptible 2", "Susceptible 3", "Susceptible 4"))
  axis(2, 0:1)
  box()}
```

```{r starting and final age distribution}
ages_h <- susceptible_h(1) + infected_h(1)
prop <- ages_h / population_h
names(prop) <- seq(1:28)
barplot(prop, main = "Starting Age Distribution")

ages_final <- out[final, 2:29] + out[final, 30:57] + out[final, 58:85] + out[final, 86:113] + out[final, 114:141] + out[final, 142:169] +
  out[final, 170:197] + out[final, 198:225] + out[final, 226:253] + out[final, 254:281] + out[final, 282:309] + out[final, 310:337]
final_population <- sum(ages_final)
prop_final <- ages_final / final_population
barplot(prop_final, main = "Final Age Distribution")
...
```{r Plotting Beta SP9}
sp9_beta <- matrix(NA, nrow = 10, ncol = 2)
colnames(sp9_beta) <- c("Beta", "SP9")

sp9_beta[9,1:2] <- c(5, SP9)
sp9_beta[10,1:2] <- c(10, SP9_l)

sp9_beta <- as.data.frame(sp9_beta)

plot(sp9_beta, type = "b", main = "SP9 and Beta Relationship")
```
