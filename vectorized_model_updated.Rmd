...
times <- seq(from = 0, to = 365 * 50, by = 0.1)
...
for(i in 0:3){
  ...
  {plot(out[,1],rowSums(out[,(1+(x-1)*28+1):(1+(x)*28)]),type='l', 
      lty= 2, col = "red",
      ylim= range(0:9e6), main = susceptible_names[i + 1])
  lines(out[,1],rowSums(out[,(1+(y-1)*28+1):(1+(y)*28)]),type='l')
  legend(300, 7e6, legend=c("High", "Low"),
...
for(i in 0:3){
  ...
  lines(out[,1],rowSums(out[,(1+(y-1)*28+1):(1+(y)*28)]),type='l')
 # legend(10000, 3000, legend=c("High", "Low"),
...
for(i in 0:3){
  ...
  {plot(out[,1],rowSums(out[,(1+(x-1)*28+1):(1+(x)*28)]),type='l', 
      lty= 2, col = "red",
      ylim= range(0:1e7), main = recovered_names[i + 1])
  lines(out[,1],rowSums(out[,(1+(y-1)*28+1):(1+(y)*28)]),type='l')
  legend(300, 7e6, legend=c("High", "Low"),
       col=c("red", "black"), lty = c(1,1), cex=0.8)}}
```
...
```{r}
matplot(sapply(1:28,function(aa)rowSums(out[,1+((1:12)-1)*28+11])),type='l',lty = 1)
```
...
for(i in 1:28){
  col <- i + 1
  S1_h_prop[i] <- out[182501,col] / sum(out[182501,2:29])
}
{barplot(S1_h_prop, main = "Proportion of Ages in S1", names.arg = axis_names)}

S2_h_prop <- rep(NA, 28)
for(i in 85:112){
  col <- i + 1
  S2_h_prop[i - 84] <- out[182501,col] / sum(out[182501,85:112])
}
{barplot(S2_h_prop, main = "Proportion of Ages in S2", names.arg = axis_names)}

S3_h_prop <- rep(NA, 28)
for(i in 169:196){
  col <- i + 1
  S3_h_prop[i - 168] <- out[182501,col] / sum(out[182501,169:196])
}
{barplot(S3_h_prop, main = "Proportion of Ages in S3", names.arg = axis_names)}

S4_h_prop <- rep(NA, 28)
for(i in 253:280){
  col <- i + 1
  S4_h_prop[i - 252] <- out[182501,col] / sum(out[182501,253:280])
}
{barplot(S4_h_prop, main = "Proportion of Ages in S4", names.arg = axis_names)}
